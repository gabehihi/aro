...
async def test_upload_bulk_invalid_extension(
    client: AsyncClient, db_session: AsyncSession
) -> None:
    """.txt 파일 → 400."""
    token = await _create_doctor(db_session)
...
async def test_upload_bulk_unauthorized(client: AsyncClient) -> None:
    resp = await client.post(
        "/api/v1/screening/upload-bulk",
        files={"file": ("results.csv", io.BytesIO(b""), "text/csv")},
    )
